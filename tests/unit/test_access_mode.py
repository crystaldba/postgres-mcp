...
from postgres_mcp.utils.sql_driver import get_sql_driver
...
async def test_get_sql_driver_returns_correct_driver(access_mode, expected_driver_type, mock_db_connection):
    ...
    with (
        patch("postgres_mcp.utils.sql_driver.current_access_mode", access_mode),
        patch("postgres_mcp.utils.sql_driver.db_connection", mock_db_connection),
    ):
...
async def test_get_sql_driver_sets_timeout_in_restricted_mode(mock_db_connection):
    ...
    with (
        patch("postgres_mcp.utils.sql_driver.current_access_mode", AccessMode.RESTRICTED),
        patch("postgres_mcp.utils.sql_driver.db_connection", mock_db_connection),
    ):
        driver = await get_sql_driver()
        assert isinstance(driver, SafeSqlDriver)
...
async def test_get_sql_driver_in_unrestricted_mode_no_timeout(mock_db_connection):
    ...
    with (
        patch("postgres_mcp.utils.sql_driver.current_access_mode", AccessMode.UNRESTRICTED),
        patch("postgres_mcp.utils.sql_driver.db_connection", mock_db_connection),
    ):
        driver = await get_sql_driver()
...
async def test_command_line_parsing():
    ...
    try:
        ...
        with (
            patch("postgres_mcp.utils.sql_driver.current_access_mode", AccessMode.UNRESTRICTED),
            patch("postgres_mcp.utils.sql_driver.db_connection.pool_connect", AsyncMock()),
            ...
            import postgres_mcp.utils.sql_driver

            postgres_mcp.utils.sql_driver.current_access_mode = AccessMode.UNRESTRICTED
            ...
            assert postgres_mcp.utils.sql_driver.current_access_mode == AccessMode.RESTRICTED
